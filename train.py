...
def get_text_data(data_path, num_samples=3000):
...
if __name__ == "__main__":
    ...
    timesteps_max, enc_tokens, characters, char2id, id2char, x, x_decoder = get_text_data(num_samples=14000,
    ...
    for _ in range(5):
        ...
        #print(np.expand_dims(x[id_from],0).shape)

        m_from, std_from = enc.predict(np.expand_dims(x[id_from],0))  #[[x[id_from]]])
        m_to, std_to = enc.predict(np.expand_dims(x[id_to],0))        #[[x[id_to]]])
        ...
        print("\n==  \t", " ".join([id2char[j] for j in np.argmax(x[id_from], axis=1)]), "==")
